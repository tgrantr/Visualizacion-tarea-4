...
Corre de principio a fin sin intervención manual:
  ...
  7. Genera HTML animado con deck.gl TripsLayer (deck.gl embebido: el
     archivo funciona sin internet y se abre con doble clic)

Outputs:
  figures/red_migracion_chile.png
  outputs/desafio_4.html
  index.html  (copia de lo anterior, para publicar en GitHub Pages)
...
URL_DECKGL     = "https://unpkg.com/deck.gl@8.9.35/dist.min.js"
...
def paso1_descarga():
    ...
    # deck.gl (se embebe en el HTML final para que no dependa de internet)
    descargar(URL_DECKGL, RAW / "deckgl-8.9.35.min.js", "deck.gl 8.9.35")
...
def paso4_animada(flujos, centroides):
    ...
    deck_js       = (RAW / "deckgl-8.9.35.min.js").read_text(encoding="utf-8")
    deck_js       = deck_js.replace("</script", "<\\/script")  # no cerrar el <script> embebido
...
setTimeout(frame, 300);
    ...
    salida = OUTPUTS / "desafio_4.html"
    salida.write_text(html, encoding="utf-8")
    print(f"  ✓ {salida} ({salida.stat().st_size/1024/1024:.1f} MB)")

    # Copia en la raíz para GitHub Pages (usa index.html como portada)
    (BASE / "index.html").write_text(html, encoding="utf-8")
    print(f"  ✓ {BASE / 'index.html'} (copia para GitHub Pages)")
...
if __name__ == "__main__":
    ...
    print(f"  HTML → {OUTPUTS / 'desafio_4.html'}  (+ index.html en la raíz)")
    print("═" * 60)
